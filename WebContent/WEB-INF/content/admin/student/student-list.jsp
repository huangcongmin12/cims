	...
	<div class="page-region ">
		<div class="page-region-content">
			<span style="text-align: center;">
				<h3><small>学生用户列表</small></h3>
			</span>
			<div>
				<a href="${pageContext.request.contextPath }/admin/student-to-add.html" class="button mini bg-color-blue fg-color-white"><i class="icon-plus-2 "></i>添加</a>
				<table class="bordered striped" >
					<thead>
						<tr>
							<th>序号</th>
							<th class="right">学号</th>
							<th class="right">真实姓名</th>
							<th class="right">性别</th>
							<th class="right">登录名称</th>
					...
					<tbody>
						<c:forEach items="${stuList}" varStatus="status" var="stu">
							<tr>
								<td>${status.count+(page.index-1) * page.pageSize}</td>
								<td class="right">${stu.studentNumber }</td>
								<td class="right">${stu.name }</td>
								<td class="right">${stu.sex.value }</td>
								<td class="right">${stu.userName }</td>
								<c:choose>
								...
								<td class="center" style="width: 102px;">
									<a href="${pageContext.request.contextPath }/admin/student-show.html?id=${stu.id}"><i class="icon-eye fg-color-blue"></i></a> 
								 	<a href="${pageContext.request.contextPath }/admin/student-to-update.html?id=${stu.id}" class=""><i class="icon-pencil fg-color-blue"></i></a> 
								 	<a id="delete" href="${pageContext.request.contextPath }/admin/student-delete.html?id=${stu.id}" onclick="if(!confirm('确定删除该信息吗？'))return false;else return true;"> <i class="icon-remove fg-color-red"></i></a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
